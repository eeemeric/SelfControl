classdef (ConstructOnLoad = false) NeuroPhysObject %< NeuroPhysExpObject
    ...
    methods
        ...
        % getSDFRaster returns rasters and histograms aligned on an event
        % of interest. 
        % INPUT ARGUMENTS
        %   whichSpike, cell array of strings or cell array of integers
        %   used to index the spike/spikes of interest
        %   trials, cell array of integer vectors used to index the trials used to produce the SDFs and rasters.
        %   event, cell array of double vectors used to align activity in the trials
        %   peritime, 1 x 2 double vector which defines the interval of time in milliseconds around the align event
        %   KernelType, integer. if 1, the SDF is convolved with a PSP
        %   filter. if 2, the SDF is convolved with a half gaussian filter
        % OUTPUT ARGUMENTS
        %   raster, n trials x max spike count double timestamps of spikes relative to the align event
        %   histogram, double vector. spike counts in 1 ms time bins (not convolved with the filter)
        %   pt, double vector. time relative to the event. is used to plot the rasters and histograms. (e.g., plot(histogram,pt), plot(raster, 1:size(raster,1),'.'), plot(histogram,SDF)
        %   SDF, double vector. Spikes per second
        	disp('Getting raster and histogram ...')
            % find spike data 
            if ~exist('whichSpike')
                [whichSpike, spikes] = obj.getSpikes;
                % preallocate output variables
                raster      = cell(length(whichSpike),1);
                histogram   = cell(length(whichSpike),1);
                pt          = cell(length(whichSpike),1);
                SDF         = cell(length(whichSpike),1);
            else
                [whichSpike, spikes] = obj.getSpikes;
                raster      = cell(length(whichSpike),1);
                histogram   = cell(length(whichSpike),1);
            ...
            for curSDF = 1:length(whichSpike)
                [raster{curSDF,1},histogram{curSDF,1},pt{curSDF,1},SDF{curSDF,1}] = getSDF...
                    (spikes{curSDF},trials,event,peritime,KernelType);
            end
        end
