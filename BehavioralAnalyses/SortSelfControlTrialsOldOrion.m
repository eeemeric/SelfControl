function [ DerivedData ] = SortSelfControlTrialsOldOrion( datafile, Behavior, SaccadeData, DerivedData,objNeuroPhys,AnatomicalInfo,SpikeInfo)
...
% if ~(exist(plxfile,'file') == 2)
%     SaccadeDetectionACCDB(datafile,TRIAL_,EYE_,EVENT_);
% end
% load (fullfile(outpath,outfile),'Sacc*','TargetOn_','Decide_')
%%
lc ='gr';
lcd ='rkbc';
ms ='o*';
...
for gg =1:size(rewards,1)
    ChFxNormal_ITC_             = zeros(length(ll_delays),3); 
    ChFxNormal_ITC_withErrors_  = zeros(length(ll_delays),3);
    
    ChFxNormal_ITC_(1:length(ll_delays),1)              = ll_delays;
    ChFxNormal_ITC_withErrors_(1:length(ll_delays),1)   = ll_delays;
    
    IT_trials_          = zeros(1,length(ll_delays)); 
    IT_error_trials_    = zeros(1,length(ll_delays)); 
    ...
    ChFx_Maintenance_= zeros(length(ll_delays),3,2);
    for hh=1:2
        switch hh
            case 1
                curChoice = choice1;
    ...
    MaintenanceTrls_    = [];%zeros(1,length(ll_delays),4);
    MaintenanceTrlIdx_  = zeros(1,length(ll_delays),4);
    choices = [1 0        
        1 1
        0 1
        0 0];
